...
async def on_ready():
    ...
    try:
        ...
        await setup_bild(bot, groq_client)
    except Exception as e:
...
async def cmd_help(ctx):
    embed = discord.Embed(
        title="VHA Bot – Befehle / Commandes",
        color=0x5865F2
    )
    ...
    embed.add_field(
        ...
        value=(
            "`!translate on` / `!translate off` – An • Aus / Activer • Désactiver\n"
            "`!translate status` – Status anzeigen / Voir le statut\n"
            "`!ai [Text]` – KI fragen / Poser une question à l'IA\n"
            "`!übersetze` / `!traduire` – Bild übersetzen / Traduire une image"
        ),
        inline=False
    )
    ...
    # ── Koordinaten ──
    embed.add_field(
        name="📍 Koordinaten / Coordonnées  🔐 R5 • R4",
        value=(
            "`!koordinaten` / `!coordonnees` – Liste anzeigen / Afficher\n"
            "`!koordinaten add NAME R X Y` – Hinzufügen / Ajouter\n"
            "`!koordinaten delete NAME` – Löschen / Supprimer"
        ),
        inline=False
    )
    ...
    # ── Timer ──
    embed.add_field(
        name="⏱️ Timer / Rappel  🔐 R5 • R4",
        value=(
            "`!timer 2h Kriegsstart` / `!rappel 2h Event` – Timer setzen / Définir\n"
