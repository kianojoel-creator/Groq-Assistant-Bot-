...
async def cmd_ai(ctx, *, question: str = None):
    ...
    lang_map = {
        "DE": ("auf Deutsch",     "Antwort auf Deutsch"),
        "FR": ("en français",     "Réponse en français"),
    }
    prompt_lang, footer = lang_map.get(lang, ("auf Deutsch", "Antwort auf Deutsch"))
    ...
    try:
        ...
        if lang == "DE":
            if reply_target_lang:
                ...
                if fr_text and fr_text.lower() != content.lower():
                    lines.append(f"🇫🇷 {fr_text}")
                if guest_text and guest_text.lower() != content.lower():
                    lines.append(f"{guest_flag} {guest_text}")
                if lines:
                ...
                fr_text = await translate_text(content, "French")
                if fr_text and fr_text.lower() != content.lower():
                    await message.reply(f"🇫🇷 {fr_text}", mention_author=False)
        ...
        # ── Fall 2: Französisch geschrieben ──────────────────────────────
            ...
            if reply_target_lang:
                ...
                de_text = await translate_text(content, "German")
                if de_text and de_text.lower() != content.lower():
                    await message.reply(f"🇩🇪 {de_text}", mention_author=False)

        # ── Fall 3: Andere Sprache (Gast) → immer DE + FR ────────────────
        else:
            de_text = await translate_text(content, "German")
            fr_text = await translate_text(content, "French")
            lines = []
