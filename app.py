...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("main_bot.log", encoding="utf-8"), logging.StreamHandler()]
)
...
# ────────────────────────────────────────────────
# LOKALE SPRACHERKENNUNG (wie beim kleinen Bot)
# ────────────────────────────────────────────────
...
async def detect_language_llm(text: str) -> str:
    """Lokale Spracherkennung - fast kein API-Call"""
    stripped = text.strip()
    ...
    lang = "OTHER"
    try:
        # Hier kannst du später langdetect installieren und nutzen
...
async def on_message(message: discord.Message):
    ...
    lang = quick_lang_detect(content) if quick_lang_detect(content) != "OTHER" else await detect_language_llm(content)
    if lang == "OTHER":
        return

    log.info(f"Übersetzung ausgelöst → Sprache: {lang} | User: {message.author}")

    # Hier kommt später deine volle translate_all Logik wieder rein
